...
const { importCustomers, downloadCustomersSample } = useBulkOperations()
...
const handleImport = async () => {
  ...
  try {
    ...
    if (result.success && result.failedRows === 0) {
      toast.success(`Successfully imported ${result.importedRows} customers`)
    } else if (result.importedRows > 0) {
      toast.warning(`Imported ${result.importedRows} customers with ${result.failedRows} failures`)
    } else {
          ...
          <div class="flex items-start gap-2">
            ...
            <div class="text-sm text-muted-foreground">
              ...
              <ul class="list-disc list-inside space-y-1">
                ...
                <li>Required columns: <code class="bg-muted px-1 rounded">customerNumber</code>, <code class="bg-muted px-1 rounded">customerType</code></li>
              </ul>
              
              <p class="font-medium text-foreground mt-3 mb-2">For INDIVIDUAL customers:</p>
              <ul class="list-disc list-inside space-y-1">
                <li><code class="bg-muted px-1 rounded">firstName</code>, <code class="bg-muted px-1 rounded">lastName</code></li>
                <li>Optional: <code class="bg-muted px-1 rounded">tcIdNo</code>, <code class="bg-muted px-1 rounded">phone</code>, <code class="bg-muted px-1 rounded">email</code></li>
              </ul>
              
              <p class="font-medium text-foreground mt-3 mb-2">For ORGANIZATIONAL customers:</p>
              <ul class="list-disc list-inside space-y-1">
                <li><code class="bg-muted px-1 rounded">organizationName</code>, <code class="bg-muted px-1 rounded">taxId</code></li>
                <li>Optional: <code class="bg-muted px-1 rounded">taxOffice</code>, <code class="bg-muted px-1 rounded">contactFirstName</code>, <code class="bg-muted px-1 rounded">contactLastName</code>, <code class="bg-muted px-1 rounded">contactPhone</code>, <code class="bg-muted px-1 rounded">contactEmail</code></li>
              </ul>
            </div>
          </div>
          
          <UiButton variant="outline" size="sm" @click="downloadCustomersSample">
      ...
      <div v-else-if="step === 'result' && importResult" class="space-y-4 py-4">
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              {{ importResult.importedRows }} of {{ importResult.totalRows }} customers imported
            </p>
          </div>
        </div>
