...
const { importMeters, downloadMetersSample } = useBulkOperations()
...
const fetchProfiles = async () => {
  try {
    ...
    console.error('Failed to fetch meter profiles:', error)
  }
}

// Profile options for select
const profileOptions = computed(() => [
  { label: 'Select a meter profile (required)', value: '' },
  ...meterProfiles.value.map(p => ({
...
const handleImport = async () => {
  ...
  try {
    const result = await importMeters({
      file: file.value,
    ...
    if (result.success && result.failedRows === 0) {
      toast.success(`Successfully imported ${result.importedRows} meters`)
    } else if (result.importedRows > 0) {
      toast.warning(`Imported ${result.importedRows} meters with ${result.failedRows} failures`)
    } else {
  ...
  <UiDialog :open="open" @update:open="handleClose">
    <UiDialogContent class="max-w-2xl">
      <UiDialogHeader>
        ...
        <UiDialogDescription>
          Import multiple meters from a CSV file
        </UiDialogDescription>
      </UiDialogHeader>
      ...
      <div v-if="step === 'form'" class="space-y-6 py-4">
        ...
        <div class="space-y-2">
          <label class="text-sm font-medium">
            Meter Profile <span class="text-destructive">*</span>
          </label>
          <UiSelect
            v-model="selectedProfileId"
            :options="profileOptions"
            placeholder="Select meter profile"
            class="w-full"
          />
          <p class="text-xs text-muted-foreground">
            All imported meters will use this profile
          </p>
        </div>
          ...
          <div class="flex items-start gap-2">
            ...
            <div class="text-sm text-muted-foreground">
              ...
              <ul class="list-disc list-inside space-y-1">
                ...
                <li>Optional columns: <code class="bg-muted px-1 rounded">initialIndex</code>, <code class="bg-muted px-1 rounded">installationDate</code>, <code class="bg-muted px-1 rounded">status</code></li>
              </ul>
            </div>
          </div>
          
          <UiButton variant="outline" size="sm" @click="downloadMetersSample">
      ...
      <div v-else-if="step === 'result' && importResult" class="space-y-4 py-4">
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              {{ importResult.importedRows }} of {{ importResult.totalRows }} meters imported
            </p>
          </div>
        </div>
      ...
      <UiDialogFooter>
        <template v-if="step === 'form'">
          ...
          >
            ...
            {{ isLoading ? 'Importing...' : 'Import Meters' }}
          </UiButton>
        </template>
        <template v-else>
