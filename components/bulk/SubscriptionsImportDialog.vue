...
const { importSubscriptions, downloadSubscriptionsSample } = useBulkOperations()
...
const handleImport = async () => {
  ...
  try {
    ...
    if (result.success && result.failedRows === 0) {
      toast.success(`Successfully imported ${result.importedRows} subscriptions`)
    } else if (result.importedRows > 0) {
      toast.warning(`Imported ${result.importedRows} subscriptions with ${result.failedRows} failures`)
    } else {
  ...
  <UiDialog :open="open" @update:open="handleClose">
    <UiDialogContent class="max-w-2xl">
      <UiDialogHeader>
        ...
        <UiDialogDescription>
          Import multiple subscriptions from a CSV file
        </UiDialogDescription>
      </UiDialogHeader>
      ...
      <div v-if="step === 'form'" class="space-y-6 py-4">
        ...
        <div class="space-y-2">
          ...
          <div
            class="border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors hover:border-primary/50 hover:bg-muted/30"
          ...
          >
            ...
            <div v-if="file" class="flex items-center justify-center gap-2 text-green-600 dark:text-green-400">
              ...
              <span class="font-medium text-lg">{{ file.name }}</span>
            </div>
        ...
        <div class="space-y-4 p-4 rounded-lg bg-muted/50">
          <div class="flex items-start gap-2">
            ...
            <div class="text-sm text-muted-foreground">
              ...
              <ul class="list-disc list-inside space-y-1">
                ...
                <li>Required columns: <code class="bg-muted px-1 rounded">subscriptionNumber</code>, <code class="bg-muted px-1 rounded">customerNumber</code></li>
              </ul>
              
              <p class="font-medium text-foreground mt-3 mb-2">Optional columns:</p>
              <ul class="list-disc list-inside space-y-1">
                <li><code class="bg-muted px-1 rounded">subscriptionGroup</code> - NORMAL_CONSUMPTION or HIGH_CONSUMPTION</li>
              </ul>
              
              <p class="font-medium text-foreground mt-3 mb-2">Address columns:</p>
              <ul class="list-disc list-inside space-y-1">
                <li><code class="bg-muted px-1 rounded">city</code>, <code class="bg-muted px-1 rounded">district</code>, <code class="bg-muted px-1 rounded">neighborhood</code></li>
                <li><code class="bg-muted px-1 rounded">street</code>, <code class="bg-muted px-1 rounded">buildingNo</code>, <code class="bg-muted px-1 rounded">floor</code>, <code class="bg-muted px-1 rounded">doorNo</code></li>
                <li><code class="bg-muted px-1 rounded">postalCode</code>, <code class="bg-muted px-1 rounded">addressCode</code></li>
                <li><code class="bg-muted px-1 rounded">latitude</code>, <code class="bg-muted px-1 rounded">longitude</code> (for map positioning)</li>
              </ul>
            </div>
          </div>
          
          <UiButton variant="outline" size="sm" @click="downloadSubscriptionsSample">
      ...
      <div v-else-if="step === 'result' && importResult" class="space-y-4 py-4">
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              {{ importResult.importedRows }} of {{ importResult.totalRows }} subscriptions imported
            </p>
          </div>
        </div>
      ...
      <UiDialogFooter>
        <template v-if="step === 'form'">
          ...
          >
            ...
            {{ isLoading ? 'Importing...' : 'Import Subscriptions' }}
          </UiButton>
        </template>
        <template v-else>
